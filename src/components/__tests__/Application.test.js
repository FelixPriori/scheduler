...
describe("Application", () =>{
  ...
  xit("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    // 4. enter the name "Student" into the input with the value "Archie Cohen"
    fireEvent.change(getByPlaceholderText(appointment, /enter student name/i), {
    ...
    // 7. Check that the element with the text "Saving" is displayed
    expect(getByText(appointment, "SAVING")).toBeInTheDocument();
    // 8. Wait until the element with the text "Student" is displayed
  ...
  xit("shows the save error when failing to save an appointment", async () => {
    ...
    // 7. Check that the element with the text "Saving" is displayed.
    expect(getByText(appointment, "SAVING")).toBeInTheDocument();
    // 8. Wait for the error element to be displayed
    await waitForElement(() => getByText(appointment, "Error"));
    // 9. Check that the error element is displayed
  ...
  xit("shows the delete error when failing to delete an existing appointment", async () => {
    ...
    // 6. Check that the element with the text "Deleting" is displayed.
    expect(getByText(appointment, "DELETING")).toBeInTheDocument();
    // 7 wait for the error element to be displayed
    await waitForElement(() => getByText(appointment, "Error"));
